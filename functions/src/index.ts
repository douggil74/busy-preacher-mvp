...
export const onPrayerHeartAdded = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const newHearts = heartsAfter.filter((heart: any) => 
      !heartsBefore.some((oldHeart: any) => oldHeart.userId === heart.userId)
    );
...
export const detectCrisis = functions.firestore
  .document('prayer_requests/{prayerId}')
...
export const updateWarriorStats = functions.firestore
  .document('prayer_requests/{prayerId}')
  .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    if (newHearts.length === 0) return null;
    ...
    for (const heart of newHearts) {
      ...
      if (userDoc.exists) {
        const totalPrayers = (userDoc.data()?.totalPrayers || 0) + 1;
        let badgeLevel = 'bronze';
        
        if (totalPrayers >= 1000) badgeLevel = 'platinum';
        else if (totalPrayers >= 100) badgeLevel = 'gold';
        else if (totalPrayers >= 25) badgeLevel = 'silver';
...
export const expireOldPrayers = functions.pubsub
